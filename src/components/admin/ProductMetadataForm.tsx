    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { key: 'suitable_for_small', label: 'Small' },
            { key: 'suitable_for_medium', label: 'Medium' },
            { key: 'suitable_for_large', label: 'Large' },
            { key: 'suitable_for_giant', label: 'Giant' }
          ].map(({ key, label }) => (
            <label key={key} className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={metadata.size_suitability[key as keyof typeof metadata.size_suitability] as boolean}
                onChange={e => setMetadata(prev => ({
                  ...prev,
                  size_suitability: {
